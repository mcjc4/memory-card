...
description: "把零散错题整理为「信号→结论」记忆卡，自动产出双版本 xlsx。当用户提到以下任何关键词时自动触发：「整理错题」「生成记忆卡」「错题本」「错题整理」「错题卡」「信号→结论」「提炼错题」「错题记忆卡」「题目整理成记忆卡」「错题整理专家」「制作错题卡」「生成错题本」「课堂错题」「物理错题」「数学错题」「化学错题」「英语错题」「高考错题」「组卷网」「zujuan.xkw.com」「错题→记忆卡」「错题 Excel」「错题 xlsx」。适用场景：错题本 Excel/截图/链接、组卷网题目（zujuan.xkw.com）、课堂纪要（docx/txt）、单题手动输入。输出：两份 xlsx（纯文本版 Unicode 公式 + LaTeX 公式版 $...$ 定界），缺一不可，物理保证。"
---
